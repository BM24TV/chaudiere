...
io.on('connection', (socket) => {
    ...
    socket.emit('updateState', { temperatures, valveMode, burnerState, heatingRequest });
...
const telnetServer = net.createServer((socket) => {
    ...
    //socket.write('Bienvenue sur le serveur Telnet des commandes de la chaudière !\r\n');
    //socket.write('Commandes disponibles : $RDTEMP[n], $GETVAN, $GETBRU, $CMDCHAU [state]\r\n');

    socket.on('data', (data) => {
        if (!socket.writable) {
            console.log('Connexion fermée, impossible d\'écrire.');
            return;
        }
